# Contribución a Nexus-Battle-Notifications
...
```text
feat/hu-55-enviar-correo-transaccional
fix/bug-12-reintento-proveedor
chore/bootstrap-sprint1-foundation
ci/cache-dependencias
docs/contrato-asyncapi
test/cobertura-consumidor
refactor/politica-reintentos
```
...
fix(notifications): [BUG-12] reintentar entrega tras fallo del proveedor
```
...
- El dominio (`src/domain`) no importa frameworks, SDK de AWS, ORM, HTTP ni drivers de base de datos.
...
- Pruebas de integración para el flujo completo del consumidor y para las sondas HTTP reales.
