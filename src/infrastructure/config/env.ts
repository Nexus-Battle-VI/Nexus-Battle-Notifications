...
export interface AuctionOutbidConfig {
  readonly port: number
  readonly secret: string
}
...
export interface CatalogNotificationsConfig {
   ...
   * HU-63.5.
   *
   * Receptor interno de Auction para registrar la notificacion
   * de puja superada dirigida por playerId.
   */
  readonly auctionOutbid: AuctionOutbidConfig | null
}
...
export const loadConfig = (env: RawEnv): AppConfig => {
  ...
  if (readBoolean(env, 'PURCHASE_HTTP_ENABLED', false)) {
    const port = readInteger(env, 'PURCHASE_HTTP_PORT', 3003, 1, 65_535)
  ...
  const auctionOutbidEnabled = readBoolean(env, 'AUCTION_OUTBID_HTTP_ENABLED', false)

  let catalogNotifications: CatalogNotificationsConfig | null = null

  if (readBoolean(env, 'CATALOG_NOTIFICATIONS_HTTP_ENABLED', false)) {
    const port = readInteger(env, 'CATALOG_NOTIFICATIONS_HTTP_PORT', 3004, 1, 65_535)
    ...
    let auctionOutbid: AuctionOutbidConfig | null = null

    if (auctionOutbidEnabled) {
      const outbidPort = readInteger(env, 'AUCTION_OUTBID_HTTP_PORT', 3005, 1, 65_535)

      const secret = readString(env, 'INTERNAL_SERVICE_AUTH_SECRET', '')

      if (secret === '') {
        throw new ConfigurationError(
          'INTERNAL_SERVICE_AUTH_SECRET es obligatorio cuando AUCTION_OUTBID_HTTP_ENABLED es "true".',
        )
      }

      if (
        outbidPort === healthPort ||
        (ingestEnabled && outbidPort === ingestPort) ||
        (purchase !== null && outbidPort === purchase.port) ||
        outbidPort === port
      ) {
        throw new ConfigurationError(
          'AUCTION_OUTBID_HTTP_PORT debe ser distinto de los otros puertos.',
        )
      }
  ...
  if (auctionOutbidEnabled && catalogNotifications === null) {
    throw new ConfigurationError(
      'AUCTION_OUTBID_HTTP_ENABLED requiere CATALOG_NOTIFICATIONS_HTTP_ENABLED="true".',
    )
  }
